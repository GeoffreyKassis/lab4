...
#define PWM_PERIOD              1000000    /* PWM period in (1 ms) */
 ...
 */
		...
		xil_printf("Starting %d\r\n", DutyCycle);
		while(1){ 											//Do forever
				for(int i = 1; i <= 50; i++){				//Cycle every 500 ms, starting at 1 because it doesnt like 0
					XTmrCtr_PwmDisable(TmrCtrInstancePtr);
					HighTime = PWM_PERIOD * i / 50;

					//xil_printf("Hightime %d\r\n", HighTime);
